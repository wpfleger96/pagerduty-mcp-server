...
def test_list_teams_with_query(mock_get_api_client, mock_teams):
    ...
    mock_get_api_client.list_all.assert_called_once_with(teams.TEAMS_URL, params={'query': query})
    assert team_list == utils.api_response_handler(results=[parse_team(result=team) for team in mock_teams], resource_name='teams')
...
def test_show_team(mock_get_api_client, mock_teams):
    ...
    assert team == utils.api_response_handler(results=parse_team(result=mock_teams[0]), resource_name='team')
