...
def test_list_schedules(mock_get_api_client, mock_schedules):
    ...
    assert schedule_list == utils.api_response_handler(results=[parse_schedule(result=schedule) for schedule in mock_schedules], resource_name='schedules')
...
def test_list_schedules_with_query(mock_get_api_client, mock_schedules):
    ...
    mock_get_api_client.list_all.assert_called_once_with(schedules.SCHEDULES_URL, params={'query': query})
    assert schedule_list == utils.api_response_handler(results=[parse_schedule(result=schedule) for schedule in mock_schedules], resource_name='schedules')
...
def test_list_schedules_empty_response(mock_get_api_client):
    ...
    schedule_list = schedules.list_schedules()
    assert schedule_list == utils.api_response_handler(results=[], resource_name='schedules')
...
def test_show_schedule(mock_get_api_client, mock_schedules):
    ...
    schedule = schedules.show_schedule(schedule_id=schedule_id, since=since, until=until)

    mock_get_api_client.jget.assert_called_once_with(f"{schedules.SCHEDULES_URL}/{schedule_id}", params={'since': since, 'until': until})
    assert schedule == utils.api_response_handler(results=parse_schedule(result=mock_schedules[0]), resource_name='schedule')
...
def test_show_schedule_no_date_params(mock_get_api_client, mock_schedules):
    ...
    mock_get_api_client.jget.assert_called_once_with(f"{schedules.SCHEDULES_URL}/{schedule_id}", params={})
    assert schedule == utils.api_response_handler(results=parse_schedule(result=mock_schedules[0]), resource_name='schedule')
...
def test_list_users_oncall(mock_get_api_client, mock_schedules):
    ...
    mock_get_api_client.jget.assert_called_once_with(f"{schedules.SCHEDULES_URL}/{schedule_id}/users", params={})
    assert users_list == utils.api_response_handler(results=[parse_user(result=user) for user in mock_users], resource_name='users')
