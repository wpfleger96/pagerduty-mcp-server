...
@pytest.mark.unit
@pytest.mark.escalation_policies
def test_list_escalation_policies_with_query(mock_get_api_client, mock_escalation_policies):
    ...
    mock_get_api_client.list_all.assert_called_once_with(escalation_policies.ESCALATION_POLICIES_URL, params={'query': query})
    assert policy_list == utils.api_response_handler(results=[parse_escalation_policy(result=policy) for policy in mock_escalation_policies], resource_name='escalation_policies')
...
def test_show_escalation_policy(mock_get_api_client, mock_escalation_policies):
    ...
    mock_get_api_client.jget.assert_called_once_with(f"{escalation_policies.ESCALATION_POLICIES_URL}/{policy_id}")
