...
def test_list_services_with_query(mock_get_api_client, mock_services):
    ...
    mock_get_api_client.list_all.assert_called_once_with(services.SERVICES_URL, params={'query': query})
    assert service_list == utils.api_response_handler(results=[parse_service(result=service) for service in filtered_services], resource_name='services')
...
def test_list_services_with_team_ids(mock_get_api_client, mock_services, mock_team_ids):
    ...
    filtered_services = [s for s in mock_services if any(tid in s.get('teams', []) for tid in mock_team_ids)]
    mock_get_api_client.list_all.return_value = filtered_services
...
def test_list_services_empty_response(mock_get_api_client):
    ...
    service_list = services.list_services()
    assert service_list == utils.api_response_handler(results=[], resource_name='services')
...
def test_show_service(mock_get_api_client, mock_services):
    ...
    mock_get_api_client.jget.assert_called_once_with(f"{services.SERVICES_URL}/{service_id}")
    assert service == utils.api_response_handler(results=parse_service(result=mock_services[0]), resource_name='service')
